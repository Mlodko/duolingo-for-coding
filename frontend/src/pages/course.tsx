...
const getTileColors = ({
  ...
  switch (status) {
    case "LOCKED":
      ...
      return "border-pink-ish bg-darker-purple";
    case "COMPLETE":
      return "border-dark-purple bg-pink-ish";
    case "ACTIVE":
      return "border-pink-ish bg-dark-purple";
  }
};
...
const TileTooltip = ({
  ...
  const activeBackgroundColor = unit?.backgroundColor ?? "bg-pink-ish";
  const activeTextColor = unit?.textColor ?? "text-pink-ish";
      ...
      <div
        className={[
          ...
          status === "ACTIVE"
            ...
            : status === "LOCKED"
              ? "border-2 border-white bg-dark-purple"
              : "bg-black",
      ...
      >
        <div
          className={[
            ...
            status === "ACTIVE"
              ? activeBackgroundColor
              : status === "LOCKED"
                ? "border-l-2 border-t-2 border-white bg-dark-purple"
                : "bg-pink-ish",
          ].join(" ")}
          style={{
        ...
        {status === "ACTIVE" ? (
          <Link
            href="/lesson"
            className={[
              "flex w-full items-center justify-center rounded-xl border-b-4 border-gray-200 bg-white p-3 hover:bg-darker-purple hover:text-white",
              activeTextColor,
            ].join(" ")}
          >
            complete for +10 XP
          ...
          <button
            className="w-full rounded-xl bg-gray-200 p-3 text-white"
            disabled
          >
            locked yet
          </button>
          ...
          <Link
            href="/lesson"
            className="flex w-full items-center justify-center rounded-xl border-b-4 border-white bg-white p-3 uppercase text-black"
          >
            practice for +5 XP
          </Link>
        )}
                ...
                switch (tile.type) {
                    ...
                    if (tile.type === "trophy" && status === "COMPLETE") {
                      return (
                        <div className="relative">
                          ...
                          <div className="absolute left-0 right-0 top-6 flex justify-center text-lg font-bold text-pink-ish">
                            {unit.unitNumber}
                          </div>
                        </div>
                      );
                  ...
                  case "treasure":
                    return (
                      ...
                      >
                        {status === "ACTIVE" && (
                          <HoverLabel text="Open" textColor="text-black" />
                        )}
                        <TileIcon tileType={tile.type} status={status} />
                      </div>
                    );
...
const getTopBarColors = (
  ...
  const defaultColors = {
    backgroundColor: "bg-darker-purple",
    borderColor: "border-pink-ish",
  } as const;
...
const course: NextPage = () => { 
  ...
  return (
    <>
      ...
      />
      <LeftBar selectedTab="course" />

      <div className="bg-darker-purple flex justify-center gap-3 pt-14 sm:p-6 sm:pt-10 md:ml-24 lg:ml-64 lg:gap-12"
        ...
        <div className="flex max-w-2xl grow flex-col">
          ...
          <div className="sticky bottom-28 left-0 right-0 flex items-end justify-between">
            <Link
              ...
              className="absolute left-4 flex h-16 w-16 items-center justify-center rounded-full border-2 border-b-4 border-pink-ish bg-white transition hover:bg-dark-purple md:left-0"
            >
            ...
            {scrollY > 100 && (
              <button
                className="absolute right-4 flex h-14 w-14 items-center justify-center self-end rounded-2xl border-2 border-b-4 border-pink-ish bg-white transition-all-1s hover:bg-dark-purple md:right-0"
    ...
    >
      <header className="flex items-center justify-between gap-4 p-4">
        ...
        <Link
          ...
          className={[
            "flex items-center gap-3 rounded-2xl border-2 border-b-4 p-3 transition hover:bg-white hover:text-pink-ish",
            borderColor,
          ].join(" ")}
        >
          <GuidebookSvg />
